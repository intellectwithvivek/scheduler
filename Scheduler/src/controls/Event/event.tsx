...
export class Event extends React.Component<IEventProps, IEventState> {
  ...
  private async onSave() {
    ...
    eventData.Description = draftToHtml(
      convertToRaw(this.state.editorState.getCurrentContent())
    );

    try {
      for (const user of this.attendees) {
        const userInfo: any = await this.spService.getUserByLoginName(
          user.id,
          this.props.siteUrl
        );
  ...
  private async renderEventData(eventId?: number) {
    ...
    const event: IEventData = !eventId
      ? this.props.event
      : await this.spService.getEvent(
          this.props.siteUrl,
          this.props.listId,
          eventId
        );
    ...
    const siteRegionalSettings =
      await this.spService.getSiteRegionalSettingsTimeZone(this.props.siteUrl);
    // chaeck User list Permissions
    const userListPermissions: IUserPermissions =
      await this.spService.getUserPermissions(
        this.props.siteUrl,
        this.props.listId
      );
    // Load Categories
    this.categoryDropdownOption = await this.spService.getChoiceFieldOptions(
      this.props.siteUrl,
      this.props.listId,
      "Category"
    );
  ...
  private onChangeEventTitle(event: any) {
    ...
    this.setState({
      eventData: { ...this.state.eventData, title: eventTitle },
    });
  }
  ...
  private onCategoryChanged(
    ...
    this.setState({
      eventData: { ...this.state.eventData, Category: item.text },
    });
  }
  ...
  private onRenderFooterContent() {
    return (
      <div>
        ...
        {this.props.panelMode == IPanelModelEnum.edit &&
          this.state.userPermissions.hasPermissionDelete && (
            <DefaultButton
  ...
  private async onEditRecurrence(ev: React.MouseEvent<HTMLButtonElement>) {
    ...
    await this.renderEventData(
      this.state.eventData.EventType == "4"
      ...
      <div>
        <Panel
          ...
          headerText={
            this.state.userPermissions.hasPermissionAdd ||
            this.state.userPermissions.hasPermissionEdit
              ? strings.EventPanelTitle
              : strings.EventPanelTitle1
          }
              ...
              <div>
                ...
                this.state.showRecurrenceSeriesInfo !== true ? (
                  <div>
                    ...
                    {this.state.recurrenceDescription ? (
                ...
                <Label>
                  {this.state.siteRegionalSettings
                    ? this.state.siteRegionalSettings.Description
                    : ""}
                </Label>
                <br />
                ...
                >
                  <Toggle
                    defaultChecked={
                      this.state.eventData && this.state.eventData.fAllDayEvent
                    }
                    inlineLabel={true}
                  ...
                  >
                    <Toggle
                      ...
                      onChange={(ev, checked: boolean) => {
                        ev.preventDefault();
                        this.setState({
                          showRecurrenceSeriesInfo: checked,
                          newRecurrenceEvent: checked,
                        });
                      }}
                    />
                  </div>
                ...
                <div>
                  <TextField
                    value={
                      this.state.eventData && this.state.eventData.location
                ...
                <div>
                  <Map
                    titleText={strings.LocationLabel}
                    coordinates={{
          ...
          {this.state.displayDialog && (
            <div>
              ...
              >
                <Label>
                  {this.state.recurrenceSeriesEdited
                    ? strings.ConfirmeDeleteAllRecurrrencesMessage
                    : strings.ConfirmeDeleteOneRecurrenceMessage}
                </Label>
                {this.state.isDeleting && (
                  <Spinner
                    size={SpinnerSize.medium}
                    ariaLabel={strings.SpinnerDeletingLabel}
                  />
                )}
                <DialogFooter>
                  <PrimaryButton
                  ...
                  <DefaultButton
                    ...
                    text={strings.DialogCloseButtonLabel}
                  />
                </DialogFooter>
              </Dialog>
            </div>
